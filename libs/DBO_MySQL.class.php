...
class DBO_MySQL extends DBO {
  ...
  private function _select($tableName, $fields = False, $count = False){
    ...
    if (isset($this->paged)) {
      $query .= " LIMIT {$this->paged[0]}, {$this->paged[1]}";
    }
  ...
  /**
   * Dirty paginator
   * See model code todos to see dirt :)
   * @param type $perPage
   * @param type $pageNum 
   */
  public function paginate($perPage, $pageNum) {
    $this->paged = array($perPage * $pageNum, $perPage * ($pageNum +1) - 1);
  }
